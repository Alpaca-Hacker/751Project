...
{
    public static class SoftBodyCubeGenerator
    {
        public static void GenerateCubeData(List<Particle> particles,
            ...
            for (var x = 0; x < res; x++)
            {
                for (var y = 0; y < res; y++)
                {
                    for (var z = 0; z < res; z++)
                    {
                        ...
                        if (x < res - 1)
                            SoftBodyGenerator.AddConstraint(particles, constraints, index, (x + 1) * res * res + y * res + z,
                                settings.structuralCompliance);
                        if (y < res - 1)
                            SoftBodyGenerator.AddConstraint(particles, constraints, index, x * res * res + (y + 1) * res + z,
                                settings.structuralCompliance);
                        if (z < res - 1)
                            SoftBodyGenerator.AddConstraint(particles, constraints, index, x * res * res + y * res + (z + 1),
                                settings.structuralCompliance);
                    }
                }
            }
            ...
            List<Constraint> constraints)
        {
            ...
            for (var x = 0; x < res - 1; x++)
            {
                for (var y = 0; y < res - 1; y++)
                {
                    ...
                    {
                        // XY face diagonals
                        SoftBodyGenerator.AddConstraint(particles, constraints,
                            x * res * res + y * res + z,
